...
open! Import
[@@@warning "-32"]
let service = Service.waf_regional
let apiVersion = "2016-11-28"
let endpointPrefix = "waf-regional"
let serviceFullName = "AWS WAF Regional"
let signatureVersion = "v4"
let protocol = "json"
let globalEndpoint = endpointPrefix ^ ".amazonaws.com"
let serviceAbbreviation = "WAF Regional"
let targetPrefix = "AWSWAF_Regional_20161128"
...
module ListResourcesForWebACLRequest =
  struct
    ...
    let make ?resourceType =
      fun ~webACLId -> fun () -> { resourceType; webACLId }
    let to_value x =
      structure_to_value
        [("WebACLId", (Some (ResourceId.to_value x.webACLId)));
        ("ResourceType",
...
module GetWebACLForResourceResponse =
  struct
    type nonrec t =
      {
      webACLSummary: WebACLSummary.t option
        [@ocaml.doc
    ...
    type nonrec error =
      ...
      | `Unknown_operation_error of (string * string option) ]
    let make ?webACLSummary = fun () -> { webACLSummary }
    let error_of_json name json =
      match name with
      | "WAFInternalErrorException" ->
          `WAFInternalErrorException (WAFInternalErrorException.of_json json)
    ...
    let to_value x =
      structure_to_value
        [("WebACLSummary",
           (Option.map x.webACLSummary ~f:WebACLSummary.to_value))]
    let to_query v = to_query to_value v
    let of_xml xml_arg0 =
      let webACLSummary =
        (Option.map ~f:WebACLSummary.of_xml)
          (Xml.child xml_arg0 "WebACLSummary") in
      make ?webACLSummary ()
    let of_string s = of_xml (Awso.Xml.parse_response s)[@@warning "-32"]
    let of_json json__ =
      let webACLSummary =
        field_map json__ "WebACLSummary" WebACLSummary.of_json in
      make ?webACLSummary ()
    let to_json v = composed_to_json to_value v
  end[@@ocaml.doc
...
module AssociateWebACLRequest =
  struct
    type nonrec t =
      {
      webACLId: ResourceId.t
        [@ocaml.doc "A unique identifier (ID) for the web ACL."];
      resourceArn: ResourceArn.t
        [@ocaml.doc
    ...
    let make ~webACLId =
      fun ~resourceArn -> fun () -> { webACLId; resourceArn }
    let to_value x =
      structure_to_value
        [("WebACLId", (Some (ResourceId.to_value x.webACLId)));
        ("ResourceArn", (Some (ResourceArn.to_value x.resourceArn)))]
