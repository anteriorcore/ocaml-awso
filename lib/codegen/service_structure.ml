...
let make_error_of_xml shapes =
  ...
  let body =
    ...
    in
    let error_e =
      [%expr `Unknown_operation_error (name, Some (Awso.Xml.to_string xml))]
    in
    List.map shapes ~f:case
    @ [ Ast_helper.Exp.case (Ast_convenience.pvar "name") error_e ]
    |> Ast_helper.Exp.match_ [%expr name]
...
let constants_of_service ~awso_service_id (s : Botodata.service) =
  let opt_item o f = Option.map o ~f in
  let loc = !Ast_helper.default_loc in
  List.filter_opt
    [ Some [%stri [@@@warning "-32"]]
    ; Some [%stri let service = [%e Botocore_service.to_ocaml awso_service_id]]
